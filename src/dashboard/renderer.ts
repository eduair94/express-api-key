 ...
 */
function generateRoleCard(role: Record<string, any>, index: number): string {
  const name = escapeHtml(role.name || 'Unknown');
  const rateLimit = role.minIntervalSeconds ?? 2;
  const monthlyQuota = role.maxMonthlyUsage ?? 10000;
  ...
  if (role.allowedEndpoints && role.allowedEndpoints.length > 0) {
    const tags = role.allowedEndpoints.map((ep: string) => `
 ...
 */
export function renderStatusPage(roles: Record<string, any>[], statusPagePath: string = '/status'): string {
